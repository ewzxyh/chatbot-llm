...
class QdrantRepository(VectorStoreRepository):
    ...
    async def delete_namespace(self, namespace_to_delete: RepositoryNamespace):
        ...
        if namespace_to_delete.engine.deployment == "local":
            ...
            qdrant_client = AsyncQdrantClient(url=namespace_to_delete.engine.host+":"+str(namespace_to_delete.engine.port),api_key=REDACTED_SECRET())
    ...
    async def delete_ids_namespace(self, engine: Engine, metadata_id: str, namespace: str):
        ...
        try:
            if engine.deployment == "local":
                ...
                qdrant_client = AsyncQdrantClient(url=engine.host+":"+str(engine.port),api_key=REDACTED_SECRET())
    ...
    async def get_ids_namespace(self, engine: Engine, metadata_id: str, namespace: str) -> RepositoryItems:
        ...
        try:
            if engine.deployment == "local":
                ...
                client = AsyncQdrantClient(url=engine.host+":"+str(engine.port),api_key=REDACTED_SECRET())
    ...
    async def create_index_old(engine, embeddings, emb_dimension) ->  QdrantVectorStore :
        ...
        if engine.deployment== "local":
            ...
            client = QdrantClient(url=engine.host+":"+str(engine.port), api_key=REDACTED_SECRET())
