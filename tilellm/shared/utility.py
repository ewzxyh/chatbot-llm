        ...
        try:
            ...
            def _creator():
                ...
                if question.llm == "openai":
                    # OLD: return ChatOpenAI(api_key=REDACTED_SECRET, model=question.model,
                    #                        temperature=question.temperature, max_tokens=question.max_tokens, top_p=question.top_p)
                    return ChatOpenAI(api_key=REDACTED_SECRET, model=question.model, **llm_params)

                elif question.llm == "anthropic":
                    # OLD: return ChatAnthropic(anthropic_api_key=REDACTED_SECRET, model=question.model,
                    #                           temperature=question.temperature, max_tokens=question.max_tokens, top_p=question.top_p)
                    return ChatAnthropic(anthropic_api_key=REDACTED_SECRET, model=question.model, **llm_params)

                elif question.llm == "cohere":
                    # OLD: return ChatCohere(cohere_api_key=REDACTED_SECRET, model=question.model,
                    #                        temperature=question.temperature, max_tokens=question.max_tokens)
                    # p è compreso tra 0.00 e 0.99
                    return ChatCohere(cohere_api_key=REDACTED_SECRET, model=question.model, **llm_params)

                elif question.llm == "google":
                    # OLD: return ChatGoogleGenerativeAI(google_api_key=REDACTED_SECRET, model=question.model,
                    #                                     temperature=question.temperature, max_tokens=question.max_tokens,
                    ...
                    return ChatGoogleGenerativeAI(google_api_key=REDACTED_SECRET, model=question.model, **llm_params)
                    ...
                    # OLD: return ChatGroq(api_key=REDACTED_SECRET, model=question.model,
                    #                      temperature=question.temperature, max_tokens=question.max_tokens, top_p=question.top_p)
                    return ChatGroq(api_key=REDACTED_SECRET, model=question.model, **llm_params)

                elif question.llm == "deepseek":
                    # OLD: return ChatDeepSeek(api_key=REDACTED_SECRET, model=question.model,
                    #                          temperature=question.temperature, max_tokens=question.max_tokens, top_p=question.top_p)
                    return ChatDeepSeek(api_key=REDACTED_SECRET, model=question.model, **llm_params)

                elif question.llm == "mistralai":
                    # OLD: return ChatMistralAI(api_key=REDACTED_SECRET, model_name=question.model,
                    #                           temperature=question.temperature, max_tokens=question.max_tokens, top_p=question.top_p)
                    return ChatMistralAI(api_key=REDACTED_SECRET, model_name=question.model, **llm_params)
...
def inject_reason_llm(func):
    ...
    async def wrapper(question, *args, **kwargs):
        try:
            ...
            def _creator():
                if question.llm == "openai":
                    return ChatOpenAI(api_key=REDACTED_SECRET,
                                      model=question.model,
                                      temperature=question.temperature,
                                      max_completion_tokens=question.max_tokens)
                elif question.llm == "anthropic":
                    return ChatAnthropic(anthropic_api_key=REDACTED_SECRET,
                                         model=question.model,
                                         temperature=question.temperature,
                                         thinking=question.thinking,
                                         max_tokens=question.max_tokens)
                elif question.llm == "deepseek":
                    return ChatDeepSeek(api_key=REDACTED_SECRET,
                                        model=question.model,
                                        temperature=question.temperature,
                                        )
                else:
...
def inject_reason_llm_old(func):
    ...
    async def wrapper(question, *args, **kwargs):
        logger.debug(question)
        if question.llm == "openai":
            chat_model = ChatOpenAI(api_key=REDACTED_SECRET,
                                    model=question.model,
                                    temperature=question.temperature,
                                    max_completion_tokens=question.max_tokens)
        elif question.llm == "anthropic":
            chat_model = ChatAnthropic(anthropic_api_key=REDACTED_SECRET,
                                       model=question.model,
                                       temperature=question.temperature,
                                       thinking=question.thinking,
                                       max_tokens=question.max_tokens)
        elif question.llm == "deepseek":
            chat_model = ChatDeepSeek(api_key=REDACTED_SECRET,
                                      model=question.model,
                                      temperature=question.temperature,
                                      #max_tokens=question.max_tokens
                                      )
