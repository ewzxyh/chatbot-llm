    ...
    def decorator(func):
        ...
        async def wrapper(self, item, *args, **kwargs):
            ...
            if item.embedding == "text-embedding-ada-002":
                ...
                embedding_obj = VoyageAIEmbeddings(voyage_api_key=REDACTED_SECRET, model="voyage-multilingual-2")
...
def inject_llm(func):
    ...
    async def wrapper(question, *args, **kwargs):
        ...
        if question.llm == "openai":
            ...
            chat_model = ChatCohere(cohere_api_key=REDACTED_SECRET,
                                    model=question.model,
                                    temperature=question.temperature,
                                    max_tokens=question.max_tokens
                                    )
            ...
            chat_model = ChatGoogleGenerativeAI(google_api_key=REDACTED_SECRET,
                                                model=question.model,
                                                temperature=question.temperature,
                                                max_tokens=question.max_tokens,
                                                top_p=question.top_p,
            ...
            chat_model = ChatGroq(api_key=REDACTED_SECRET,
                                  model=question.model,
                                  temperature=question.temperature,
                                  max_tokens=question.max_tokens,
                                  top_p=question.top_p
...
def inject_llm_chat(func):
    ...
    async def wrapper(question, *args, **kwargs):
        ...
        if question.embedding == "openai":
            ...
            llm_embeddings = OpenAIEmbeddings(api_key=REDACTED_SECRET, model=question.embedding.embedding_model)
